# Project Carousel USB Anniversary Edition (PCUAE)
...
You can now load PCUAE on the Atari2600 Plus and load ROM(.a26/.bin 2600 files and .a78/.bin 7800 files) Games on it from the USB Drive instead of cartridges, you can still run cartridges too, you can run Stella Emulator on it too and run cartridges in Stella as well.
...
PCUAE is completely free and you do not need to pay for it or charged to use it, if you do then you are been ripped off, it free to download and use, make sure you always download it from here or from the PCUAE Official Website below.
...
If you just installed PCUAE and this has popped up then no need to read it, do it in your own time.. :) and read only the parts you need too.
...
PCUAE Updater has now been added to PCUAE Network Mode, if you have PCUAE connected to the internet now and is using a Ethernet Adapter with THEC64 Models/THEA500, then go into the PCUAE System Menu to check if PCUAE has any updates available, its been added to PCUAE 3.3.5.
...
Then run PCUAE and select Amiga Mode>AmigaSYS4 3.1 WinUAE Version then pick `Resolution UAE:1280x720 RGBA` then you be able to use `IBrowse 2.4`(`IBrowse 2.5` is included, its in the other2 Amiga Hard Drive on the Workbench, you can run it from there too or move it where you want it) and load webpages in it.
...
## PCUAE Automatically Updating the Firmware (TheCarousel in THEA500 Only)

It changes TheCarousel in the firmware to the one used in TheCarousel Changer(it temporary changes it) so if you change from Manahttan v1.0.0 to Manahttan v1.2.1, it will use v1.2.1 in the firmware too, it can be downgraded too, It can be disabled as well in the PCUAE System Menu, it default is on automatically so switches TheCarousel in the firmware(when you remove the USB Drive and load the Firmware) to the same as TheCarousel used in PCUAE Mode.
...
You can add your own games in its carousel, a set of games(for C64 and VIC20) are now included with PCUAE All In One Version and are included with PCUAE Manager too so you can export them to the PCUAE USB Drive, to enable PCUAE Mode on THEC64 Models you need to export over Favorites 1 Gameslist to THEC64-CAROUSEL-GAMES folder on the root of THE PCUAE USB Drive for it to see the gameslists you add, you can add what games you like in the Favorites 1 Gamelist in PCUAE Manager, it will need games adding to it if the carousel see no games at all it will not load the carousel, if favorites 1 is not added then a blue screen will appear and say there is no Favorites 1 Gamelist and then load the Firmware carousel with its default games instead of PCUAE Mode.
...
You can load VICE Mode and load C64 games on THEC64 and THEA500. (the pack is available on here)

You can connect to the internet and load up BBS borads in VICE Mode in THEC64.(THEC64 Only)

You can load Amiga(16bit), Atari(8bit), Vice(C16/plus4,C64,VIC20,C128,8bit) and RetroArch(RA) Modes on THEC64 and THEA500, THEA500 can run RetroAami Mode instead of RetroArch Mode, they are now all in one installer now under Retroarch Mode Pack(the packs are available on here).
...
Media Access - `USB stick/drive` content will appear on the screen... :).. BINGO... :D

![Media Access](https://i.ibb.co/YZx66Wp/Screenshot-2023-08-09-153911.jpg)
...
A lot of work has gone in to this version to make sure everything runs as it should, thanks Davy for the help, fixing scripts and that, PCUAE is now to big for one person to work on, it would take to long to release it between versions.

## New PCUAE Official Website, finally here... https://projectcarouselusb.eu

I changed my mind about having a site, it needs one, downloads are available there too and other goodies.. :) you have to register on the site to download files and have access to the site, if you use the PCUAE site then you will not need to use the PCUAE Menu.
...
It looks like RA is using a French keyboard layout, AZERTY keyboard, a keyboard layout used in France and neighboring countries.
...
What the keys are meant to be, QWERTY Keyboard(English)
...
I do not know what the number pad is like because I do not have a Keyboard with a number pad.
...
I will fix this as soon as I can, once I have figured it out, you can still play Pinball games OK, the shift keys have not moved or the other keys like: CTRL, A(Amiga key/ALT) and Tab keys and can still use the keyboard or you can use the Virtual Keyboard too if you like.
...
* "PCUAE now contains host-rum from the Pandory500 mod"... https://github.com/emuchicken/pandory-a500
  ...
  It will from now on include host-rum.adf file from Pandory so it can boot PCUAE using it, thanks Dajoho and Team Pandory for allowing me to use it... :)
* I will always try to support PCUAE, but do need a break from it from time to time... :)
* WIKI is here, it might need updating so might not included new features or info on the new version of PCUAE - PCUAE Manual is here - https://github.com/CommodoreOS/PCUAE/wiki
...
THEA500 Mini has been blocked from doing it by RGL so you can not add or do anything with THEA500 Mini's TheCarousel, it will never be possable to do anything with it, sorry this is not my fault, RGL has made the carousel on purpose this way, so you can not change its games so its impossible, this is why now the next machine do not interest me now, because they be doing this to all the machines(Devices) they make and you be stuck with the games on the carousel that you will hardly play because you can't change them, TheCarousel is just for show and made to make the GUI look nice and to sell it, there is no purpose to the carousel really and its not future prove, once you played the games on it a few times then you will not use it more, if you could add games to it, it be more usefull, its kind of obvious and you have no need for a USB Media Menu on the machines.

You can change to different Modes(Emulators) on THEA500 Mini in PCUAE only but can not add games to its Carousel like you can with THEC64, RGL(Retro Games Ltd) have built the carousel this way on purpose so you can not add games to its carousel, probably cause they are thinking about how much money they are making from THEA500 more but thats why they made the machines in the first place, to make money from it and I guess to make sure they do not lose money, I would not care if anyone modded it cause it helps selling it, like what happen to the PS Classic, hacking made it a better console and users started to buy it cause of Bleemsync.
