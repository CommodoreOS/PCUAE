...
On THEA500 Mini when you remove the PCUAE USB Drive it marks it so makes it think it needs scaning for errors, this is down to RGL not ejecting the USB Drive properly in THEA500 Mini so will always mark the USB Drive and make Windows thing it has errors on it when it does not, you can scan it for error and it will say it found none, becuase its false, it does it all the time, its a bug that RGL has never fixed, they need to tell THEA500 Linux System to stop marking the drive for false errors, this is not down too PCUAE, it does it even if you use the USB Drive for just Amiga Games so RGL need to fix it, whats the point of RGL providing away to update THEA500 Mini then if they never fix anything in it, they should of issued a upade over 6 months a ago, its been over a year now and its still stuck on v1.1.1, well they got there money from it now so on to the next machine, they might as well not adding a way to update the next machine, becuse there is not point to having it if your not going to update it of fix its probems, like: some games do only work using Amiberries Autoboot lha, not all games load and some games run slow and with broken graphics so THEA500 Mini is not 100% and it should be for a paid product and not have gliches, they should of learn from making THEC64.
